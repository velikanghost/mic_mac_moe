...
import { FiLogOut, FiXCircle, FiPlayCircle } from 'react-icons/fi'
...
export default function Home() {
  ...
  const connectWallet = async () => {
    ...
    setCachedSigner(signer) // Cache the signer to avoid multiple MetaMask popups
  }
  ...
  useEffect(() => {
    if (currentGame) {
      ...
      const moveListener = (gameId: any, player: string, position: number) => {
        console.log('MoveMade event detected:', { gameId, player, position })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1F2937] to-[#4F46E5] text-white">
      <div className="container mx-auto py-12 px-4">
        ...
        <div className="max-w-lg mx-auto bg-[#111827] rounded-3xl p-8 shadow-lg">
          {walletAddress ? (
            <div className="mb-6 flex justify-between items-center">
              ...
              >
                <FiLogOut className="inline-block mr-2" /> Disconnect
              </Button>
            </div>
          ) : (
            <div className="text-center mb-6">
              <Button
                ...
                className="bg-[#5FEDDF] text-[#111827] hover:bg-opacity-80"
              >
                Connect Wallet
              </Button>
            </div>
